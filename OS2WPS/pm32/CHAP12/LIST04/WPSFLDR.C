...
#define FCF_WPS         FCF_HIDEBUTTON | FCF_STANDARD & ~FCF_MINBUTTON
                                ...
                                MPARAM mp2)
{
    ...
    switch( msg)
    {
                ...
                case FID_CLIENT:
                    switch( SHORT2FROMMP( mp1))
                    {
                        ...
                        case CN_INITDRAG:
                        {
                            ...
                            drgitem.hstrSourceName = DrgAddStrHandle( pcnrdraginit -> pRecord -> pszIcon) ;
                            drgitem.hstrTargetName = DrgAddStrHandle( pcnrdraginit -> pRecord -> pszIcon) ;
                            drgitem.cxOffset = 0 ;
                            drgitem.cyOffset = 0 ;
                            drgitem.fsControl = 0 ;
                        ...
                        case CN_DRAGOVER:
                        {
                            ...
                            pcnrdraginfo = (PCNRDRAGINFO)mp2 ;
                            pdraginfo = (PDRAGINFO)pcnrdraginfo -> pDragInfo ;
                            prec = (PRECORDCORE)pcnrdraginfo -> pRecord ;
                            papprec = (PAPPREC)prec ;

                            // forbid dropping on R-G-B objects
                            //if( papprec -> precParent)
                        ...
                        case CN_DROP:
                        {
                            ...
                            // only RGB objects on empty space
                            if( !prec && !pappsource -> lType)
                            {
                                WinAlarm( HWND_DESKTOP, WA_NOTE) ;
                                prec = CreateAnotherObj( CTRL( hwnd, FID_CLIENT), 1L, NULL, 0L) ;

                                // create three objects
                                CreateAnotherObj( CTRL( hwnd, FID_CLIENT), OBJECTS, prec, -1L) ;
                            }
                            else
                            {
                                CreateAnotherObj( CTRL( hwnd, FID_CLIENT), 1,  prec, pappsource -> lType) ;
                            }
                        }
                            break ;
                        ...
                        case CN_EMPHASIS:
                        {
                            PNOTIFYRECORDEMPHASIS pntrecemph ;
                            PAPPREC papprec ;
                            PRECORDCORE prec ;

                            pntrecemph = (PNOTIFYRECORDEMPHASIS)mp2 ;

                            // emphasize the whole container ;
                            if( !pntrecemph -> pRecord)
                            {
                            }
                        }
                            break ;
        ...
        case WM_SYSCOMMAND:
            ...
            {
                case SC_CLOSE:
                    WinSendMsg( CTRL( hwnd, FID_CLIENT), WM_CLOSE, 0L, 0L) ;
                    break ;
            }
            break ;
                                ...
                                MPARAM mp2)
{
    ...
    switch( msg)
    {
        ...
        case WM_STARTUP:
        {
            ...
            cnrinfo.pszCnrTitle = "Drag & Drop Container" ;
